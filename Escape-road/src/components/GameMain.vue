  ...
  <div class="game" :class="{ 'theater-mode': isTheaterMode }">
    ...
    <div class="top-bar">
      <div class="top-bar-left">
        <img :src="getImageUrl(gameData.image)" :alt="gameData.title" class="top-bar-image" />
        <!-- 1. 移动 Rating 到按钮前面 -->
      ...
      <div class="controls">
        <button
          ...
          :disabled="!iframeSrc || !isIframeLoaded"
        >
          {{ theaterButtonText }}
        </button>
        <button
...
const fullscreenButtonText = computed(() =>
  isFullscreen.value ? 'Exit the Fullscreen' : 'Fullscreen'
)
...
/* Adjust top-bar to handle potentially 3 items - Keep this if needed for layout */
/* .top-bar { ... } */
