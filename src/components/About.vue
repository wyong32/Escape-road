...
// 动态计算的 max-height 样式
const contentMaxHeightStyle = ref({ maxHeight: '500px' }) // 初始折叠高度
...
watch(isExpanded, async (newValue) => {
  ...
  if (contentRef.value) {
    if (newValue) {
      ...
      // 收起时，恢复初始高度
      contentMaxHeightStyle.value = { maxHeight: '500px' }; // 确保与 CSS 初始值一致
    }
  }
...
watch(() => props.gameId, () => {
  ...
  contentMaxHeightStyle.value = { maxHeight: '500px' }; // 重置高度
}, { immediate: true })
